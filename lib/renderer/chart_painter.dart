...
class ChartPainter extends BaseChartPainter {
  ...
  void drawRightText(canvas) {
    var textStyle =
        getTextStyle(chartColors.yAxisTextColor, chartStyle.defaultTextSize);
    ...
    for (var i = 0; i <= chartStyle.gridColumns; ++i) {
      ...
      if (translateX >= startX && translateX <= stopX) {
        ...
        TextPainter tp = getTextPainter(
            DataUtil.getDate(datas![index].id, chartStyle.dateFormatter),
  ...
  void drawCrossLineText(Canvas canvas, Size size) {
    ...
    TextSpan priceSpan = formatPrice(
        point.close, getTextStyle(Colors.white, chartStyle.defaultTextSize));
    TextPainter tp =
        TextPainter(text: priceSpan, textDirection: TextDirection.ltr);
    tp.layout();
    ...
    TextPainter dateTp = getTextPainter(
        DataUtil.getDate(point.id, chartStyle.dateFormatter),
        color: Colors.white);
  ...
  void drawMaxAndMin(Canvas canvas) {
    ...
    TextStyle minMaxStyle =
        getTextStyle(chartColors.maxMinTextColor, chartStyle.defaultTextSize);
    if (x < mWidth / 2) {
      //画右边
  ...
  void drawRealTimePrice(Canvas canvas, Size size) {
    ...
    TextSpan priceSpan = formatPrice(
        point.close,
        getTextStyle(chartColors.rightRealTimeTextColor,
            chartStyle.realTimeTextSize ?? chartStyle.defaultTextSize));
    TextPainter tp =
        TextPainter(text: priceSpan, textDirection: TextDirection.ltr);
    tp.layout();
    // 价格内间距
    ...
    if (tp.width < max) {
      ...
      TextSpan priceSpan = formatPrice(
          point.close,
          getTextStyle(chartColors.realTimeTextColor,
              chartStyle.realTimeTextSize ?? chartStyle.defaultTextSize));
  ...
  TextPainter getTextPainter(text, {color = Colors.white}) {
    ...
    TextSpan span = TextSpan(
        text: "$text", style: getTextStyle(color, chartStyle.defaultTextSize));
  ...
  bool shouldRepaint(covariant ChartPainter oldDelegate) {
    return super.shouldRepaint(oldDelegate) ||
        ...
        oldDelegate.secondaryStates != secondaryStates ||
