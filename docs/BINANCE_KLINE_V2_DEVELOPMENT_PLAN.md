...
- 单人开发：约 9～12 周。
- 两人并行：约 6～8 周，其中渲染与指标可以在数据模型冻结后并行。
- 建议额外预留 20% 缓冲，用于跨平台差异、性能回归和 API 兼容。

若第一版仅要求“币安基础 K 线体验”，完成 Phase 0～5 即可，单人约 5～7 周。
...
### Phase 0：冻结基线与测量，2～3 人日

- [ ] `P0-01` 保存当前公共 API 清单和 Demo 截图。
- [ ] `P0-02` 建立 100/2,000/10,000 根标准数据集。
- [ ] `P0-03` 为现有 MA、EMA、BOLL、SAR、MACD、KDJ、RSI、WR、VOL、OBV 建立结果基线。
- [ ] `P0-04` 建立拖动、缩放、十字线的 profile 性能基线。
- [ ] `P0-05` 建立 Android/iOS/Web 构建矩阵。
- [ ] `P0-06` 输出 2.0 API 草案和 deprecated 清单。

阶段门禁：有可重复性能数据和视觉基线，禁止在没有基线的情况下改写 Painter。

### Phase 1：数据模型与实时合并器，4～6 人日

- [ ] `P1-01` 实现不可变 Kline、Interval、PriceSource。
- [ ] `P1-02` 实现旧 `KLineEntity` adapter。
- [ ] `P1-03` 实现历史 replace/prepend/append/update。
- [ ] `P1-04` 实现重复、乱序、闭合和 generation token 规则。
- [ ] `P1-05` 实现 Binance REST/WebSocket 示例 adapter，但核心包不依赖 Binance API。
- [ ] `P1-06` 完成 10,000 根数据合并 benchmark。

阶段门禁：模型单元测试通过；实时事件幂等；旧 Demo 可通过 adapter 运行。

### Phase 2：独立指标引擎，6～8 人日

- [ ] `P2-01` 定义 Indicator、IndicatorConfig、IndicatorSeries 协议。
- [ ] `P2-02` 实现缓存和增量更新协议。
- [ ] `P2-03` 迁移现有指标并做标准数据对照。
- [ ] `P2-04` 增加 VWAP、ATR、CCI、DMI、ROC、Stoch RSI。
- [ ] `P2-05` 增加多实例和数据不足状态。
- [ ] `P2-06` 删除新渲染链路对实体 `late` 指标字段的依赖。

阶段门禁：公式误差在约定范围；最后一根 K 线更新满足 8 ms 预算；不得出现 NaN/Infinity 泄漏到 Renderer。

### Phase 3：视口与分层渲染，7～10 人日

- [ ] `P3-01` 实现 ChartViewport 和坐标双向转换。
- [ ] `P3-02` 实现网格、主图、副图、标签、十字线独立 Layer。
- [ ] `P3-03` 实现可见区裁剪和极值缓存。
- [ ] `P3-04` 迁移蜡烛、分时、面积图。
- [ ] `P3-05` 实现实心/空心与 Heikin-Ashi。
- [ ] `P3-06` 实现最新价、倒计时、最大最小标记。
- [ ] `P3-07` 建立多尺寸、多主题 Golden。
- [ ] `P3-08` 完成 2,000 根 + 2 副图帧性能门禁。

阶段门禁：Golden 通过；P95 帧时间达标；旧组件可选择 legacy 或 v2 Renderer。

### Phase 4：Controller 与交互，5～7 人日

- [ ] `P4-01` 实现 KChartController。
- [ ] `P4-02` 实现焦点缩放、惯性平移和视口复位。
- [ ] `P4-03` 实现十字线、磁吸和详情事件。
- [ ] `P4-04` 实现历史分页状态机。
- [ ] `P4-05` 实现回到最新和指定时间定位。
- [ ] `P4-06` 实现横屏、鼠标和父级滚动手势策略。
- [ ] `P4-07` 完成手势自动化与输入延迟测试。

阶段门禁：不存在多个图表共享滚动状态；十字线移动不重绘数据层；手势测试稳定。

### Phase 5：币安式核心体验与主题，4～6 人日

- [ ] `P5-01` 完整 KChartTheme 和兼容 ChartColors adapter。
- [ ] `P5-02` 周期和图表类型工具栏示例。
- [ ] `P5-03` 指标选择、排序和参数面板示例。
- [ ] `P5-04` 多副图布局与高度配置。
- [ ] `P5-05` 行情摘要、OHLC 信息栏和全屏 Demo。
- [ ] `P5-06` 用户配置序列化接口。

阶段门禁：Phase 0～5 构成可单独发布的 `2.0.0-alpha`，具备完整基础交易图表体验。

### Phase 6：绘图工具，8～12 人日

- [ ] `P6-01` 绘图对象、锚点、样式和版本化 JSON。
- [ ] `P6-02` 绘图命中测试和控制点。
- [ ] `P6-03` 实现 BN-D01～BN-D04 工具。
- [ ] `P6-04` 实现磁吸、移动、锁定和删除。
- [ ] `P6-05` 实现 Command 模式撤销/重做。
- [ ] `P6-06` 实现跨周期恢复策略。
- [ ] `P6-07` 完成绘图 Golden 和序列化兼容测试。

阶段门禁：缩放、分页、旋转屏幕后锚点稳定；绘图 Overlay 不影响基础帧预算超过 10%。

### Phase 7：交易叠加与深度图，5～8 人日

- [ ] `P7-01` 通用 PriceLine、Marker 和 EventOverlay。
- [ ] `P7-02` 仓位、强平、挂单、止盈止损示例。
- [ ] `P7-03` 交易叠加点击和拖动回调。
- [ ] `P7-04` 重构深度模型与累计曲线。
- [ ] `P7-05` 实现深度快照/增量合并和失步事件。
- [ ] `P7-06` 深度高频更新性能测试。
...
- [ ] `P8-01` 全量 unit/widget/golden/integration/benchmark。
- [ ] `P8-02` Android/iOS/Web profile 和 release 构建。
- [ ] `P8-03` 无障碍、RTL、时区和国际化检查。
- [ ] `P8-04` API 文档、迁移指南和完整 Example。
- [ ] `P8-05` 发布 `dev → alpha → beta → rc → stable`。
- [ ] `P8-06` 每个预发布版本至少保留一轮真实项目接入反馈。
...
| 功能持续扩张拖延 2.0 | 高 | 高 | Phase 0～5 先发布，绘图和交易叠加后置 |
...
| 版本 | 包含阶段 | 交付目标 |
| --- | --- | --- |
| `1.1.x` | Phase 0～2 的兼容部分 | 稳定数据与指标，保留当前 UI |
| `2.0.0-alpha` | Phase 0～5 | 新内核和币安式核心 K 线 |
| `2.0.0-beta` | Phase 6 | 绘图工具可用 |
| `2.0.0-rc` | Phase 7～8 | 交易叠加、深度和跨平台验收 |
...
1. `P0-01`～`P0-06`：建立不可丢失的行为与性能基线。
2. `P1-01`：评审新 Kline 模型，不立即删除旧实体。
3. `P2-01`：在模型冻结后定义指标协议。

第一轮不得直接开发绘图工具或新增大量指标。先证明新模型、增量计算和分层渲染能够满足性能预算，再进入上层功能开发。
