...
- [x] `BN-D01` 趋势线、水平线、垂直线和射线。
- [x] `BN-D02` 矩形、平行通道。
- [x] `BN-D03` 斐波那契回撤。
- [x] `BN-D04` 文本和价格标记。
- [x] `BN-D05` OHLC 磁吸。
- [x] `BN-D06` 选中、移动、控制点、锁定和删除。
- [x] `BN-D07` 撤销和重做。
- [ ] `BN-D08` 显示/隐藏所有绘图。
...
- [x] `P7-02` 绘图命中测试和控制点。
- [x] `P7-03` 实现 BN-D01～BN-D04 工具。
- [x] `P7-04` 实现磁吸、移动、锁定和删除。
- [x] `P7-05` 实现 Command 模式撤销/重做。
- [x] `P7-06` 实现跨周期恢复策略。
- [x] `P7-07` 完成绘图 Golden 和序列化兼容测试。
...
- 状态：已完成交易所无关的深度快照事件、增量事件、版本化本地盘口和实时协调器。
- 合并：增量可插入、更新和以零数量删除价格档位；提交后重新标准化买卖排序并拒绝交叉盘口。
- 连续性：`u <= localId` 的旧事件不推进版本；事件区间覆盖 `localId + 1` 时可桥接；`U > localId + 1` 或精确续接时 `pu != localId` 进入失步状态。
- 缓冲：快照到达前的增量按 generation 和交易对隔离并受容量上限保护；快照会丢弃已覆盖事件、重放连续事件，并保留无法桥接的事件等待更新快照。
- 恢复：失步保留最后有效盘口且发布 `requiresSnapshot`；新快照若覆盖缺口，可重放缓冲事件恢复同步；generation 切换清空旧请求和旧流事件。
- Demo：深度区域新增“模拟正常增量”“模拟丢包”“重新同步”中文按钮和同步状态，覆盖完整恢复闭环。
...
- 口径：AVL 使用每根 Binance K 线的 `quoteVolume / baseVolume`，SUPER 使用 Wilder ATR(10) 与乘数 3 的追踪上下轨，并在反转点断开不同趋势的绘制序列。
