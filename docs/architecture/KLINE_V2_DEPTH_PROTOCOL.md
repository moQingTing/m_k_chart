...
P8-06 新增 `DepthCurveSamplingPolicy`，且默认保持不裁剪、不采样：

1. 每侧先按 `maxRetainedLevelsPerSide` 保留最接近买一/卖一的档位，最外侧低优先级档位被裁剪；
2. 对保留档位完整计算累计数量，再按 `maxRenderedPointsPerSide` 等距采样累计点；
3. 采样始终保留首档和最外保留档，所以最优价、可见价格跨度和最外累计量不会因抽点丢失；
4. `DepthCurve` 暴露源档数、保留档数及是否发生裁剪/采样，方便宿主监控降载结果；
5. `DepthCurveCache` 使用盘口对象 identity + 策略作为键，避免命中时对 2,000 档执行结构比较；LRU 容量有界且由每个图表实例持有。

中文 Demo 持有 1,000 买档 + 1,000 卖档，并配置为每侧最多绘制 160 点。Host 基准连续执行 600 次事件，Android Profile 宿主按真实 100 ms 定时器执行 10 Hz 更新。完整数据与复现命令见 `docs/PERFORMANCE_P8_DEPTH_GATE.md`。

## 7. 后续边界

P8-04～P8-06 已冻结静态模型、累计数学、纯渲染、同步恢复和高频降载。以下内容不在本阶段内：
...
- 裁剪先于累计、等距采样保留首尾、累计量不丢失；
- 大盘口绘制点数上限和 identity LRU 命中/淘汰；
- 1,000×2 档、10 Hz Host 与 Android Profile 性能门禁；
- 完整 Flutter 回归保持通过。
