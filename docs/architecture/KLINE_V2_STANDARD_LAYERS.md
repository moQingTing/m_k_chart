...
| 5 | `marker` | 可见高低点、最新价线和标签 | data、viewport、layout、theme、clock |
| 6 | `tradeOverlay` | 价格线、数值标记、时间/价格事件点 | data、viewport、layout、theme、overlays |
| 7 | `drawing` | chart-local 与时间/价格锚定绘图 | data、viewport、layout、theme、drawings |
| 8 | `crosshair` | chart-local 十字线和价格标签 | selection、layout、theme |
...
- TradeOverlay 从 Snapshot 的不可变交易对象绘制主图价格线、值标记和时间/价格事件点；三类对象共享 ID 命名空间，按买入/卖出/中立语义映射主题颜色，并严格裁剪在主图 panel；
...
- marker/tradeOverlay/drawing/crosshair 分色独立输出；
